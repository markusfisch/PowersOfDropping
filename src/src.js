...
function gameLost() {
	...
	player.alive = false
	shake()
	zzfx(...[5,0,129.81,.02,.34,.26,1,1.7,,-0.1,,.01,,,,,,.8,.17,,781])
}
...
function impact(x, y) {
	zzfx(...[1.8,,59,.01,.03,.04,3,3.5,,-3,,,,1.4,,.2,.04,.65,.02])
	shake()
	spawnDust(x, y)
	setItem(x, y, BLOCK)
...
function digestItem(e) {
	...
	items[o] = 0
	zzfx(...[,,584,.02,.17,.36,,2,,,53,.06,.09,,,.2,,.9,.25])
}
...
function clearWallAt(x, y) {
	...
	score += 1
	items[o] = pickItem()
	zzfx(...[1.2,,428,,.09,.16,4,1.8,-1,,,,,.2,,.5,,.48,.01,,1123])
	spawnDust(x, y, 4)
	shake()
}
